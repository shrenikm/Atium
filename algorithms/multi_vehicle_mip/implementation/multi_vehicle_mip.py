...
def construct_dynamics_constraints(
    solver: Solver,
    ...
    next_time_step_id = current_time_step_id + 1

    assert current_time_step_id >= 0
...
def construct_constraints_for_mvmip(
    ...
    for vehicle_id, vehicle in enumerate(vehicles):
        ...
        for current_time_step_id in range(nt):
            dyn_cons_map = construct_dynamics_constraints(
                solver=solver,
                vars_map=vars_map,
                vehicle_id=vehicle_id,
                vehicle=vehicle,
            ...
            # Add dyn_cons_map to cons_map, but make sure that we never have any duplicates or they'll be overwritten.
            for dyn_cons_var in dyn_cons_map.keys():
                assert dyn_cons_var not in cons_map
            cons_map.update(dyn_cons_map)
